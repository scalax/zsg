...
class Test58 extends AnyFunSpec with Matchers {
  case class Foo58(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap58 = PropertyApply[Foo58]
  import in._
  import out._
  val fooEncoder58: ListEncoder[Foo58]         = encoder
  val reverseFooEncoder58: ListEncoder[Foo58]  = reverseEncoder
  val fooDecoder58: ModelDecoder[Foo58]        = decoder
  val reverseFooDecoder58: ModelDecoder[Foo58] = reverseDecoder
  describe("A case class by 58 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder58.encode(fooValue58).mkString("|")
      str1 shouldBe fooValue58.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder58.encode(fooValue58).mkString("|")
      str2 shouldBe fooValue58.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder58.getData(fooValue58.toString)
      model1 shouldBe fooValue58
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder58.getData(fooValue58.reverseString)
      model2 shouldBe fooValue58
    }
  }
}
