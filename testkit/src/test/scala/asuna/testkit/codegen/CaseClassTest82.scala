...
class Test82 extends AnyFunSpec with Matchers {
  case class Foo82(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap82 = PropertyApply[Foo82]
  import in._
  import out._
  val fooEncoder82: ListEncoder[Foo82]         = encoder
  val reverseFooEncoder82: ListEncoder[Foo82]  = reverseEncoder
  val fooDecoder82: ModelDecoder[Foo82]        = decoder
  val reverseFooDecoder82: ModelDecoder[Foo82] = reverseDecoder
  describe("A case class by 82 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder82.encode(fooValue82).mkString("|")
      str1 shouldBe fooValue82.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder82.encode(fooValue82).mkString("|")
      str2 shouldBe fooValue82.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder82.getData(fooValue82.toString)
      model1 shouldBe fooValue82
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder82.getData(fooValue82.reverseString)
      model2 shouldBe fooValue82
    }
  }
}
