...
class Test66 extends AnyFunSpec with Matchers {
  case class Foo66(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap66 = PropertyApply[Foo66]
  import in._
  import out._
  val fooEncoder66: ListEncoder[Foo66]         = encoder
  val reverseFooEncoder66: ListEncoder[Foo66]  = reverseEncoder
  val fooDecoder66: ModelDecoder[Foo66]        = decoder
  val reverseFooDecoder66: ModelDecoder[Foo66] = reverseDecoder
  describe("A case class by 66 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder66.encode(fooValue66).mkString("|")
      str1 shouldBe fooValue66.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder66.encode(fooValue66).mkString("|")
      str2 shouldBe fooValue66.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder66.getData(fooValue66.toString)
      model1 shouldBe fooValue66
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder66.getData(fooValue66.reverseString)
      model2 shouldBe fooValue66
    }
  }
}
