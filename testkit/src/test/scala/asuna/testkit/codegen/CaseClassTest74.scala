...
class Test74 extends AnyFunSpec with Matchers {
  case class Foo74(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap74 = PropertyApply[Foo74]
  import in._
  import out._
  val fooEncoder74: ListEncoder[Foo74]         = encoder
  val reverseFooEncoder74: ListEncoder[Foo74]  = reverseEncoder
  val fooDecoder74: ModelDecoder[Foo74]        = decoder
  val reverseFooDecoder74: ModelDecoder[Foo74] = reverseDecoder
  describe("A case class by 74 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder74.encode(fooValue74).mkString("|")
      str1 shouldBe fooValue74.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder74.encode(fooValue74).mkString("|")
      str2 shouldBe fooValue74.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder74.getData(fooValue74.toString)
      model1 shouldBe fooValue74
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder74.getData(fooValue74.reverseString)
      model2 shouldBe fooValue74
    }
  }
}
