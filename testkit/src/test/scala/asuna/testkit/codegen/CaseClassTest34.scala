...
class Test34 extends AnyFunSpec with Matchers {
  case class Foo34(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap34 = PropertyApply[Foo34]
  import in._
  import out._
  val fooEncoder34: ListEncoder[Foo34]         = encoder
  val reverseFooEncoder34: ListEncoder[Foo34]  = reverseEncoder
  val fooDecoder34: ModelDecoder[Foo34]        = decoder
  val reverseFooDecoder34: ModelDecoder[Foo34] = reverseDecoder
  describe("A case class by 34 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder34.encode(fooValue34).mkString("|")
      str1 shouldBe fooValue34.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder34.encode(fooValue34).mkString("|")
      str2 shouldBe fooValue34.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder34.getData(fooValue34.toString)
      model1 shouldBe fooValue34
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder34.getData(fooValue34.reverseString)
      model2 shouldBe fooValue34
    }
  }
}
