...
class Test18 extends AnyFunSpec with Matchers {
  case class Foo18(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap18 = PropertyApply[Foo18]
  import in._
  import out._
  val fooEncoder18: ListEncoder[Foo18]         = encoder
  val reverseFooEncoder18: ListEncoder[Foo18]  = reverseEncoder
  val fooDecoder18: ModelDecoder[Foo18]        = decoder
  val reverseFooDecoder18: ModelDecoder[Foo18] = reverseDecoder
  describe("A case class by 18 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder18.encode(fooValue18).mkString("|")
      str1 shouldBe fooValue18.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder18.encode(fooValue18).mkString("|")
      str2 shouldBe fooValue18.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder18.getData(fooValue18.toString)
      model1 shouldBe fooValue18
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder18.getData(fooValue18.reverseString)
      model2 shouldBe fooValue18
    }
  }
}
