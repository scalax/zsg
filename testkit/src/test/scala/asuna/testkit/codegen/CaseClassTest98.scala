...
class Test98 extends AnyFunSpec with Matchers {
  case class Foo98(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap98 = PropertyApply[Foo98]
  import in._
  import out._
  val fooEncoder98: ListEncoder[Foo98]         = encoder
  val reverseFooEncoder98: ListEncoder[Foo98]  = reverseEncoder
  val fooDecoder98: ModelDecoder[Foo98]        = decoder
  val reverseFooDecoder98: ModelDecoder[Foo98] = reverseDecoder
  describe("A case class by 98 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder98.encode(fooValue98).mkString("|")
      str1 shouldBe fooValue98.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder98.encode(fooValue98).mkString("|")
      str2 shouldBe fooValue98.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder98.getData(fooValue98.toString)
      model1 shouldBe fooValue98
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder98.getData(fooValue98.reverseString)
      model2 shouldBe fooValue98
    }
  }
}
