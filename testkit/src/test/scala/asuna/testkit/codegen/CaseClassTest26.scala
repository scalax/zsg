...
class Test26 extends AnyFunSpec with Matchers {
  case class Foo26(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap26 = PropertyApply[Foo26]
  import in._
  import out._
  val fooEncoder26: ListEncoder[Foo26]         = encoder
  val reverseFooEncoder26: ListEncoder[Foo26]  = reverseEncoder
  val fooDecoder26: ModelDecoder[Foo26]        = decoder
  val reverseFooDecoder26: ModelDecoder[Foo26] = reverseDecoder
  describe("A case class by 26 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder26.encode(fooValue26).mkString("|")
      str1 shouldBe fooValue26.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder26.encode(fooValue26).mkString("|")
      str2 shouldBe fooValue26.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder26.getData(fooValue26.toString)
      model1 shouldBe fooValue26
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder26.getData(fooValue26.reverseString)
      model2 shouldBe fooValue26
    }
  }
}
