...
class Test50 extends AnyFunSpec with Matchers {
  case class Foo50(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap50 = PropertyApply[Foo50]
  import in._
  import out._
  val fooEncoder50: ListEncoder[Foo50]         = encoder
  val reverseFooEncoder50: ListEncoder[Foo50]  = reverseEncoder
  val fooDecoder50: ModelDecoder[Foo50]        = decoder
  val reverseFooDecoder50: ModelDecoder[Foo50] = reverseDecoder
  describe("A case class by 50 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder50.encode(fooValue50).mkString("|")
      str1 shouldBe fooValue50.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder50.encode(fooValue50).mkString("|")
      str2 shouldBe fooValue50.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder50.getData(fooValue50.toString)
      model1 shouldBe fooValue50
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder50.getData(fooValue50.reverseString)
      model2 shouldBe fooValue50
    }
  }
}
