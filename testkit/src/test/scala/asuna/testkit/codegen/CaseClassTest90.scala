...
class Test90 extends AnyFunSpec with Matchers {
  case class Foo90(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap90 = PropertyApply[Foo90]
  import in._
  import out._
  val fooEncoder90: ListEncoder[Foo90]         = encoder
  val reverseFooEncoder90: ListEncoder[Foo90]  = reverseEncoder
  val fooDecoder90: ModelDecoder[Foo90]        = decoder
  val reverseFooDecoder90: ModelDecoder[Foo90] = reverseDecoder
  describe("A case class by 90 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder90.encode(fooValue90).mkString("|")
      str1 shouldBe fooValue90.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder90.encode(fooValue90).mkString("|")
      str2 shouldBe fooValue90.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder90.getData(fooValue90.toString)
      model1 shouldBe fooValue90
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder90.getData(fooValue90.reverseString)
      model2 shouldBe fooValue90
    }
  }
}
