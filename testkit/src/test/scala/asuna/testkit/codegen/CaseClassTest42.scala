...
class Test42 extends AnyFunSpec with Matchers {
  case class Foo42(
    i1: Long,
    i2: Int,
    i3: Int,
  ...
  val ap42 = PropertyApply[Foo42]
  import in._
  import out._
  val fooEncoder42: ListEncoder[Foo42]         = encoder
  val reverseFooEncoder42: ListEncoder[Foo42]  = reverseEncoder
  val fooDecoder42: ModelDecoder[Foo42]        = decoder
  val reverseFooDecoder42: ModelDecoder[Foo42] = reverseDecoder
  describe("A case class by 42 length") {
    it("should generic to a encoder") {
      val str1 = fooEncoder42.encode(fooValue42).mkString("|")
      str1 shouldBe fooValue42.toString
    }
    it("should generic to a reverse encoder") {
      val str2 = reverseFooEncoder42.encode(fooValue42).mkString("|")
      str2 shouldBe fooValue42.reverseString
    }
    it("should generic to a decoder") {
      val (_, model1) = fooDecoder42.getData(fooValue42.toString)
      model1 shouldBe fooValue42
    }
    it("should generic to a reverse decoder") {
      val (_, model2) = reverseFooDecoder42.getData(fooValue42.reverseString)
      model2 shouldBe fooValue42
    }
  }
}
