...
class EndParameter extends TypeParameter {
  override type H = EndParameter
  override type T = EndParameter
}

object EndParameter {
  val endParameter: EndParameter = new EndParameter
}
...
trait Context[K <: KindContext] {
  ...
  def start: K#M[org.scalax.asuna.mapper.EatXyyType0]
  ...
  def reverse: Context[K] = new Context[K] {
    ...
    override def start: K#M[org.scalax.asuna.mapper.EatXyyType0]                                                                    = self.start
  }
