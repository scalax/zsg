...
import asuna.macros.{DefaultValue, PropertyTag}
import asuna.{Application, Context, TypeParameter2, TypeParameter3}
import io.circe._

trait CircePoly {

  implicit def imEncoder[T](implicit t: => Encoder[T]): Application[EncoderTest.KContext, PropertyTag[T], TypeParameter2[String, T]] =
    new Application[EncoderTest.KContext, PropertyTag[T], TypeParameter2[String, T]] {
  ...
  implicit def imDecoder[T](implicit dd: => Decoder[T]): Application[DecoderTest.KM, PropertyTag[T], TypeParameter3[String, T, DefaultValue[T]]] =
