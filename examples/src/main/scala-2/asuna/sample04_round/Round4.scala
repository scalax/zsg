...
object MM {
  ...
  def implicitP2[F[_, _], T1, T2](i1: CContext[F, T1, T2]): CContext[F, AsunaTuple1[T1], AsunaTuple1[T2]] = {
    new CContext[F, AsunaTuple1[T1], AsunaTuple1[T2]] {
      override def append[Y1, Y2, Z1, Z2](i: F[Y1, Y2], p: Plus2[Y1, Y2, AsunaTuple1[T1], AsunaTuple1[T2], Z1, Z2]): F[Z1, Z2] = {
        i1.append(
          i,
          new Plus2[Y1, Y2, T1, T2, Z1, Z2] {
            override def takeHead1(z: Z1): Y1    = p.takeHead1(z)
            override def takeTail1(z: Z1): T1    = p.takeTail1(z).i1
            override def plus1(x: Y1, y: T1): Z1 = p.plus1(x, new AsunaTuple1(y))
            override def takeHead2(z: Z2): Y2    = p.takeHead2(z)
            override def takeTail2(z: Z2): T2    = p.takeTail2(z).i1
            override def plus2(x: Y2, y: T2): Z2 = p.plus2(x, new AsunaTuple1(y))
          }
        )
      }

      override def map[Z1, Z2](p: Map2[AsunaTuple1[T1], AsunaTuple1[T2], Z1, Z2]): F[Z1, Z2] = {
        i1.map(new Map2[T1, T2, Z1, Z2] {
          override def take1(z: Z1): T1 = p.take1(z).i1
          override def plus1(x: T1): Z1 = p.plus1(new AsunaTuple1(x))
          override def take2(z: Z2): T2 = p.take2(z).i1
          override def plus2(x: T2): Z2 = p.plus2(new AsunaTuple1(x))
        })
      }
    }
  }
