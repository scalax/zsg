...
class Abc extends FlatSpec with Matchers
  ...
  val categories1111 = Fetcher.relOnly(
    { (repo: ProductRepo, ids: RelationIds[Any]) =>
      ...
      val query = friendTq3.map { friend =>
        val cc = new UmrReaderQuery[List[(Any, FriendTable3 => Any)]] {
  ...
  val fetcher1 = Fetcher { (ctx: ProductRepo, reps: Seq[FriendTable3 => SlickShapeValueWrap[Any]]) =>
    val query = friendTq3.map { friend =>
      val cc = new UmrReaderQuery[List[(Any, FriendTable3 => SlickShapeValueWrap[Any])]] {
        override def umrSv: DataShapeValue[List[(Any, FriendTable3 => SlickShapeValueWrap[Any])], SlickShapeValueWrap[Any]] = reps.map(s => s.apply(friend).mapReader(t => t -> s)).toList.shaped
      }
      UmrReaderQuery(cc)
    }.to[List]
    val action = query.result
    println("11" * 20 + action.statements)
    db.run(action.head)
  }(HasId(s => s._2))
  ...
  lazy val ProductType = ObjectType("aa", "cc",
    fields[ProductRepo, Product](
      ...
      Field("name", ListType(StringType), resolve = { c =>
        ...
        DeferredValue(fetcher1.deferSeq(List(s => rep(s.nick).map(s => s: Any)))).map(_.map(_._1.asInstanceOf[String]))
        /*aabb.future*/ /*.map { t =>
          println("ee" * 100)
          t
        }*/
      ...
      Field("description", StringType,
        ...
        resolve = { c =>
          ...
          DeferredValue(fetcher1.defer(s => rep(s.nick).map(s => s: Any))).map(_._1.asInstanceOf[String])
        }),
      Field("friendId", LongType,
        description = Some("Picture CDN URL"),
        resolve = { c =>
          //categories.deferRelSeq(byParent, c.value.description.size)
          //categories1111.deferRel(r2.asInstanceOf[Relation[Any, (Any, FriendTable3 => Any), FriendTable3 => MappedProjection[(Any, FriendTable3 => Any), String]]], r2.anyShapeValue.asInstanceOf[FriendTable3 => MappedProjection[(Any, FriendTable3 => Any), String]]).asInstanceOf[Deferred[String]]
          DeferredValue(fetcher1.defer(s => rep(s.id).map(s => s: Any))).map(_._1.asInstanceOf[Long])
        }),
