...
class NodeTag10[
  ...
  override type XyyItemType = Item10[
    T1#XyyItemType,
    T2#XyyItemType,
    T3#XyyItemType,
    T4#XyyItemType,
    ...
    new org.scalax.asuna.mapper.Application[
      ...
      override def application(context: org.scalax.asuna.mapper.Context[K]): K#M[EatXyyType10[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10]] = {
        if (context.isReverse) {
          ...
          context.append(NodeTag9.appendNodeTagEatXyy9(t1, t2, t3, t4, t5, t6, t7, t8, t9).application(context), t10.application(context), ArticleXyyPlus10.plus10)
        }
      }
    }
}
class `Please change .debug(context) to .find10.debug(context).` extends org.scalax.asuna.mapper.item.Message
