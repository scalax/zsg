...
object AppendTag {
  final val tag                            = new AppendTag[Any]
   ...
   inline  final def lift[T](tag : => AppendTag[T]): AppendTag[T] = AppendTag[T]
       ...
       inline  final def mergeTag[T1 <: TupleTag  , T2 <: TupleTag , Target <: TupleTag](t1: AppendTag[T1]  , t2 : AppendTag[T2] )( given  i: Merge2[T1  , T2 , Target]): AppendTag[Target] = AppendTag[Target]
