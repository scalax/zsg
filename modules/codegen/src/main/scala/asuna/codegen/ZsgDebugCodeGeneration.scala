...
object ZsgDebugCodeGeneration {
  ...
  val root2XDir = Paths.get(".").resolve("modules").resolve("debug").resolve("src").resolve("main").resolve("scala-2").resolve("zsg").resolve("codegen")

  val rootAllDir = Paths.get(".").resolve("modules").resolve("debug").resolve("src").resolve("main").resolve("scala").resolve("zsg").resolve("codegen")

  val rootDottyDir = Paths.get(".").resolve("modules").resolve("debug").resolve("src").resolve("main").resolve("scala-3").resolve("zsg").resolve("codegen")
  ...
  def main(i: Array[String]): Unit = {

    {
      for (proNum <- 1 to 64) yield {
        ...
        val content = StringUtil.trimLines(zsg.codegen.debug.txt.DebugCaseClass(maxPropertyNum = ZsgParameters.maxPropertyNum)(proNum = proNum)(isDotty = false).body)
      ...
      for (proNum <- 1 to 64) yield {
        ...
        Files.createDirectories(filePath.getParent)
        val writer  = new PrintWriter(filePath.toFile, "utf-8")
        val content = StringUtil.trimLines(zsg.codegen.debug.txt.DebugCaseClass(maxPropertyNum = ZsgParameters.maxPropertyNum)(proNum = proNum)(isDotty = true).body)
        writer.println(content)
        writer.close()
      }
    }
