...
object SealedTraitSelector {
  ...
  implicit final def asunaCirceSealedEncoder[T, R](
    ...
    new Application3[SealedTraitSelector[T]#JsonEncoder, SealedTag[R], Class[R], String] with con.JsonEncoder[SealedTag[R], Class[R], String] {
      override def application(context: Context3[SealedTraitSelector[T]#JsonEncoder]): SealedTraitSelector[T]#JsonEncoder[SealedTag[R], Class[R], String] = this
