...
object ScalaTupleContext extends Context[TupleEncFun] {
...
object tuple {
  def asString[T, H <: TypeHList](x: T)(implicit ii: Application[TupleEncFun, T, H { type Head = T }]): String = {
    val encoder = new TupleEncoder[T, H#Tail#Head] {
      override def body(t: List[String], i: T): List[String] =
        ii.application(ScalaTupleContext).body(List.empty, i).mkString("(", ",", ")") :: t
  ...
  class TupleGen[T] {
    def fromString[H <: TypeHList](str: String)(implicit ii: Application[TupleEncFun, T, H]): H#Tail#Head = {
      val decoder = new TupleEncoder[H#Head, H#Tail#Head] {
        override def body(t: List[String], i: H#Head): List[String] =
          ii.application(ScalaTupleContext).body(List.empty, i).mkString("(", ",", ")") :: t
        override def stringBody(i: H#Head): String = ii.application(ScalaTupleContext).body(List.empty, i).mkString("(", ",", ")")
        override def fromString(str: String): (H#Tail#Head, String) = {
          val (t, str1) = ii.application(ScalaTupleContext).fromString(str.dropWhile(s => s == '(' || s == ',' || s == ')'))
          val str2      = str1.dropWhile(s => s == '(' || s == ',' || s == ')')
          (t, str2)
...
trait AppendTuple {
  ...
  implicit def applicationImplicit[T](implicit i: TupleEncoder[T, T]): Application[TupleEncFun, T, TypeAlias.TypeHList2[T, T]] =
    new Application[TupleEncFun, T, TypeAlias.TypeHList2[T, T]] {
