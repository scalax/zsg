...
object CaseClassHelperGen1 extends App {
  ...
  val rootDir        = Paths.get("./").resolve("core").resolve("src").resolve("main").resolve("scala").resolve("org").resolve("scalax").resolve("asuna").resolve("mapper").resolve("codegen")
  ...
  for (i <- 2 to maxPropertyNum) yield {
    ...
    val content191 = Source.fromString(org.scalax.asuna.template.txt.ItemTag(tagNum = i).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  ...
  for (i <- 2 to maxPropertyNum) yield {
    ...
    Files.createDirectories(path19.getParent)
    val writer19   = new PrintWriter(path19.toFile, "utf-8")
    val content191 = Source.fromString(org.scalax.asuna.template.txt.NodeTag(tagNum = i).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
    val content192 = content191.mkString(System.lineSeparator)
    writer19.println(content192)
    writer19.close()
  }
  ...
  for (i <- 2 to maxPropertyNum) yield {
    ...
    Files.createDirectories(path19.getParent)
    val writer19   = new PrintWriter(path19.toFile, "utf-8")
    val content191 = Source.fromString(org.scalax.asuna.template.txt.ItemTypeParameterPlus(tagNum = i).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
    val content192 = content191.mkString(System.lineSeparator)
    writer19.println(content192)
    writer19.close()
  }

  val path21 = rootDir.resolve("ii").resolve("ItemTypeParameter.scala")
  Files.createDirectories(path21.getParent)
  val writer21   = new PrintWriter(path21.toFile, "utf-8")
  val content211 = Source.fromString(org.scalax.asuna.template.txt.ItemTypeParameter(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  val content212 = content211.mkString(System.lineSeparator)
  writer21.println(content212)
  writer21.close()

  val path22 = rootDir.resolve("ii").resolve("TypeParameter.scala")
  Files.createDirectories(path22.getParent)
  val writer22   = new PrintWriter(path22.toFile, "utf-8")
  val content221 = Source.fromString(org.scalax.asuna.template.txt.TypeParameter(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  val content222 = content221.mkString(System.lineSeparator)
  writer22.println(content222)
  writer22.close()

  val path23 =
    rootDir.resolve("ii").resolve("NoteNumberMessage.scala")
  Files.createDirectories(path23.getParent)
  val writer23   = new PrintWriter(path23.toFile, "utf-8")
  val content231 = Source.fromString(org.scalax.asuna.template.txt.NoteNumberMessage(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  val content232 = content231.mkString(System.lineSeparator)
  writer23.println(content232)
  writer23.close()

  val path24 =
    rootDir.resolve("ii").resolve("ItemNumberMessage.scala")
  Files.createDirectories(path24.getParent)
  val writer24   = new PrintWriter(path24.toFile, "utf-8")
  val content241 = Source.fromString(org.scalax.asuna.template.txt.ItemNumberMessage(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  ...
  val path10 =
    rootDir.resolve("ii").resolve("tuple").resolve("EatXyyType.scala")
  ...
  val path11 = rootDir.resolve("ii").resolve("tuple").resolve("TupleApplicationImplicits.scala")
  ...
  val path18 = rootDir.resolve("ii").resolve("BuildContent.scala")

  Files.createDirectories(path18.getParent)

  val writer18 = new PrintWriter(path18.toFile, "utf-8")
  val content181 =
    Source.fromString(org.scalax.asuna.template.txt.BuildContent(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
