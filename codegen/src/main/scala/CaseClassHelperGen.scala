...
object CaseClassHelperGen1 extends App {
  ...
  val path9 = tupleDir.resolve("TupleTypeParameterPlus1.scala")
  Files.createDirectories(path9.getParent)
  val writer9   = new PrintWriter(path9.toFile, "utf-8")
  val content91 = Source.fromString(org.scalax.asuna.tuple.template.txt.TupleTypeParameterPlus1().body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  val content92 = content91.mkString(System.lineSeparator)
  writer9.println(content92)
  writer9.close()

  val path19 = tupleDir.resolve("TupleTypeParameterPlus2.scala")
  Files.createDirectories(path19.getParent)
  val writer19   = new PrintWriter(path19.toFile, "utf-8")
  val content191 = Source.fromString(org.scalax.asuna.tuple.template.txt.TupleTypeParameterPlus2().body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  val content192 = content191.mkString(System.lineSeparator)
  writer19.println(content192)
  writer19.close()

  val path20 = tupleDir.resolve("TupleTypeParameterPlus3.scala")
  Files.createDirectories(path20.getParent)
  val writer20   = new PrintWriter(path20.toFile, "utf-8")
  val content201 = Source.fromString(org.scalax.asuna.tuple.template.txt.TupleTypeParameterPlus3().body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
