...
object CaseClassHelperGen1 extends App {
  ...
  val content111 =
    Source.fromString(org.scalax.asuna.template11.tuple.txt.AppendTuple(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  ...
  val content181 =
    Source.fromString(org.scalax.asuna.template11.tag.txt.BuildTag(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
