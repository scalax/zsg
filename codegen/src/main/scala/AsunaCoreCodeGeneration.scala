...
object AsunaCoreCodeGeneration extends App {
  ...
  for (i <- 2 to maxPropertyNum) yield {
    ...
    val content191 = Source.fromString(asuna.tuple.template.txt.TupleTag(tagNum = i).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  ...
  for (i <- 2 to maxPropertyNum) yield {
    ...
    Files.createDirectories(path19.getParent)
    val writer19   = new PrintWriter(path19.toFile, "utf-8")
    val content191 = Source.fromString(asuna.tuple.template.txt.NodeTag(tagNum = i).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
    val content192 = content191.mkString(System.lineSeparator)
    writer19.println(content192)
    writer19.close()
  }
  ...
  for (i <- 2 to maxPropertyNum) yield {
    ...
    Files.createDirectories(path19.getParent)
    val writer19   = new PrintWriter(path19.toFile, "utf-8")
    val content191 = Source.fromString(asuna.tuple.template.txt.HListToTupleTypeHListPlus(tagNum = i).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
    val content192 = content191.mkString(System.lineSeparator)
    writer19.println(content192)
    writer19.close()
  }
  ...
  for (i <- 2 to maxPropertyNum - 1) yield {
    ...
    Files.createDirectories(path19.getParent)
    val writer19   = new PrintWriter(path19.toFile, "utf-8")
    val content191 = Source.fromString(asuna.tuple.template.txt.AsunaTuple(tagNum = i).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
    val content192 = content191.mkString(System.lineSeparator)
    writer19.println(content192)
    writer19.close()
  }
  ...
  for (i <- 1 to maxPropertyNum) yield {
    ...
    Files.createDirectories(path19.getParent)
    val writer19   = new PrintWriter(path19.toFile, "utf-8")
    val content191 = Source.fromString(asuna.tuple.template.txt.AsunaValueTuple(tagNum = i).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
    val content192 = content191.mkString(System.lineSeparator)
    writer19.println(content192)
    writer19.close()
  }
  ...
  for (i <- 2 to maxPropertyNum - 1) yield {
    ...
    Files.createDirectories(path19.getParent)
    val writer19   = new PrintWriter(path19.toFile, "utf-8")
    val content191 = Source.fromString(asuna.tuple.template.txt.HListTypeHListPlus(tagNum = i).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
    val content192 = content191.mkString(System.lineSeparator)
    writer19.println(content192)
    writer19.close()
  }
  ...
  val content211 =
    Source.fromString(asuna.tuple.template.build.txt.TupleTypeHList(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  ...
  val content221 =
    Source.fromString(asuna.tuple.template.build.txt.TypeHList(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  ...
  val content231 =
    Source.fromString(asuna.tuple.template.build.txt.NoteNumberMessage(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  ...
  val content241 =
    Source.fromString(asuna.tuple.template.build.txt.TupleNumberMessage(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  ...
  val content181 =
    Source.fromString(asuna.tuple.template.build.txt.BuildContent(maxItem = maxPropertyNum).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
  ...
  val content161 = Source.fromString(asuna.tuple.template.build.txt.HListTypeHList(maxPropertyNum - 1).body).getLines.toList.map(_.trim).filter(s => !s.isEmpty)
