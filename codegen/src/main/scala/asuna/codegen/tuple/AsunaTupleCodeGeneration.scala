...
object AsunaTupleCodeGeneration {
  def main(i: Array[String]): Unit = {
    val maxTupleNum = 22
    val rootDir =
      Paths.get("./").resolve("scala-tuple").resolve("src").resolve("main").resolve("scala").resolve("asuna").resolve("tuple")
    val supportDir =
    ...
    {
      val filePath = rootDir.resolve("ScalaTupleHListTypeHList.scala")
      Files.createDirectories(filePath.getParent)
      val writer = new PrintWriter(filePath.toFile, "utf-8")
      val content =
